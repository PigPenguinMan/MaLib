...
import { ButtonHTMLAttributes, Suspense, useEffect, useState } from "react";
...
const BoardMain = () => {
  ...
  // 11/14 pageNm 최대치에 갔을때 다음 페이지버튼 disabled 필요 API에서 불러오는 length가 8이하일때 disabled 시키기 ?
  useEffect(()=>{
    const prevbtn  = document.getElementById('board_prevbtn') as HTMLButtonElement;
    const nextbtn = document.getElementById('board_nextbtn');
    if(pageNm <= 0 ){
    ...
    */
    <div className="flex flex-col items-center ">
      ...
      <div className="board_pagebtnwrap flex gap-4 pb-2">
        <button onClick={()=>{setPageNm(pageNm-1)}} id="board_prevbtn" className="rounded-md border hover:border-slate-100" ><Image src="/prevbtn.svg" alt="이전페이지 아이콘" width={25} height={25}/></button>
        <button onClick={()=>{setPageNm(pageNm+1)}} id="board_nextbtn" className="rounded-md border hover:border-slate-100"><Image src="/nextbtn.svg" alt="다음페이지 아이콘" width={25} height={25}/></button>
      </div>
    </div>
  );
};
